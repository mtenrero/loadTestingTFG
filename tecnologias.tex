...
\begin{enumerate}
  \item \st{\textbf{Lenguaje Golang:} Carácteristicas básicas del lenguaje, cross-platform, cobertura de test}
  \item \st{\textbf{Git, Travis \& Jenkins:} Diferencias y features críticas que me han hecho pasar de Travis a Jenkins}
  \item \st{\textbf{OpenAPI}}
  \item \st{\textbf{Goa Design:} Paquete Golang, Design First API, creo que es importante por el carácter de piensa antes de picar, muy similar a TDD}
  \item \textbf{AWS} Toda la CI está desplegada en AWS y tengo pensado montar un cluster Swarm aquí para una futura demo.
  \item \st{\textbf{Docker}}
  \item \st{\textbf{ElasticSearch} Características básicas}
  \item \st{\textbf{Shared Filesystem:} GlusterFS / Samba . No es Kubernetes, asi que se necesita esta configuración previa para tener todos los ficheros disponibles en todo el cluster. Explicar lo que ofrece y por qué es necesario.}
  \item \st{\textbf{Redes:} Concretamente DNS y algoritmo DNS Round Robin. Es la piedra angular en cuanto a orquestación con arquitectura Master / Workers se refiere}
  \item \st{\textbf{Apache Jmeter:} Forma de ejecución distribuida y arquitectura}
\end{enumerate}
...
El lanzamiento del nodo Maestro requiere que se especique la lista de los nodos remotos que utilizar para lanzar el test de rendimiento.
...
Usa ficheros YAML para su configuración y todos los comandos y procesos indicados en los ficheros de configuración se ejecutan sobre contenedores completamente aislados. Ofrecen Docker-In-Docker de manera limitada, únicamante para construir imágenes y publicarlas en un registro.
