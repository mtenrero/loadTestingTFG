...
Para conseguir la capacidad de gestión de las versiones de dependencias a nivel de proyecto, dep descarga todas las dependencias en el directorio local \textit{vendor} para que estén disponibles en el propio proyecto, al estilo del directorio \textit{node\_modules} en el caso de NodeJS.\newline

Esta distribución de ficheros evita al compilador de \textit{Go} tener que buscar en el \$GOPATH las dependencias necesarias.
...
Es un framework enfocado al diseño de la \gls{API} en primer lugar, y es lo que hace a este framework único. Posee su propio lenguaje \gls{DSL} (lenguaje descriptivo) en el que antes de codificar, obliga al programador a pensar en el diseño de la \gls{API}, ya que esta debe ser definida en un primer lugar. Permite definir desde el endpoint, el contenido que consumirá y los parámetros que recibirá.\newline

Dispone de un comando generador, \textit{goagen}, que produce todo el código necesario a partir de la descripción proporcionada en los ficheros de definición. No está completamente integrado con \textit{dep} y para el correcto funcionamiento del comando \textit{goagen} se requiere que no exista la carpeta \textit{vendor}.
...
LXC es un tipo de virtualización basada en contenedores, usa el espacio de nombres de kernel y cgroups para asegurar el aislamiento de los contenedores, aunque comparten el kernel con el Sistema Operativo del host \cite{ContainersComparison}.

\subsection{Docker}

Docker añade una capa para gestionar las redes de los contenedores dotándolos de una IP propia y sistema de ficheros a LXC. Permite especificar imágenes de contenedores autocontenidas y gestionar el ciclo de vida de los mismos. 
...
\section{\gls{AWS}: Amazon Web Services}

Amazon Web Services es una solución Platform-as-a-Service (\gls{PAAS}) que ofrece diversos productos, entre ellos:
